@if (!(Auth::user() && Auth::user()->id == $recipe->creator->id))
    <div class="modal fade" id="reviewFormContainer" aria-hidden="true">
        <div class="modal-dialog modalContainer modal-content"
            style="max-width: 80vw; width:60vw; height:90vh; background-color:white">

            <div>
                <img src="/assets/icons/close_icon.png" alt="close_icon" class="picon closeModalBtn" style="float:right"
                    onclick="$('#reviewFormContainer').modal('hide')">
            </div>
            <div class="modalContainerContent">
                <div class="text-center justify-content-center">
                    ...
                    <img src="/assets/chef_review.png" style="height:150px; width:auto; border-radius:50%"
                        alt="chef_review">
                </div>


                <form action="/submitReview" id="reviewForm" enctype="multipart/form-data" style="padding:30px"
                    method="POST">
                    @csrf
                    <div class="col">
                        <label for="rating">Berikan bintang untuk resep ini</label><label
                            style="color:rgb(255, 57, 57)">*</label>
                        <div class="rating">
                            @for ($i = 1; $i <= 5; $i++)
                                <img src="/assets/icons/empty_star.png" id="star_{{ $i }}"
                                    onclick="adjustStar({{ $i }})" class="starIcon" alt="star_icon">
                            @endfor
                        </div>
                        <input type="hidden" id="reviewRating" name="rating" value=""></input>
                    </div>
                    <div class="col">
                        <label for="comment">Apa tanggapanmu mengenai resep ini?</label><label
                            style="color:rgb(255, 57, 57)">*</label>
                        <textarea maxlength="250" class="form-control textField whiteBackground" placeholder="Tulis tanggapanmu disini"
                            oninput="recountTotalChar('reviewComment', 'reviewTanggapanTotalChar')" id="reviewComment" name="comment"
                            rows="3"></textarea>
                        <div style="float:right">
                            <span id="reviewTanggapanTotalChar">0</span>
                            <span">/250</span>
                        </div>
                    </div>
                    <div class="col" style="display:flex; flex-direction: column">
                        <label for="img">Unggah foto masakanmu :</label>
                        <input type="file" id="reviewImg" name="img" onchange="switchModal(this)"
                            accept="image/*"></input>
                    </div>
                    ...
                    <input type="hidden" name="creator_id" value="{{ $recipe->creator->id }}"></input>
                </form>
            </div>
        </div>
    </div>
